...
- **Administrators and developers who need to log in to the homepage of the Discovery Service**
    ...
    Some utilities including HTTPie require the certificate to be in PEM format located in `keystore/localhost/localhost.pem`.
    Since the Discovery Service uses HTTPS, your client also requires verification of the validity of its certificate. Verification is performed by trusting the local CA certificate stored at `keystore/local_ca/localca.cer`.
